...
void AuroraeTheme::borders(int& left, int& top, int& right, int& bottom, bool maximized) const
{
    ...
    if (maximized) {
        ...
        left = qBound(minMargin, (int)left - d->themeConfig.borderLeft(), maxMargin);
        right = qBound(minMargin, (int)right - d->themeConfig.borderRight(), maxMargin);
        bottom = qBound(minMargin, (int)bottom - d->themeConfig.borderBottom(), maxMargin);
